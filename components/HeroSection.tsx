...
export const HeroSection = () => (
  <section className="relative overflow-hidden border-b border-zinc-900 bg-white py-16 md:py-24">
    <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
      <div>
        <div className="mb-7 inline-flex rotate-[-4deg] border border-amber-500 bg-amber-50 px-3 py-1 text-[10px] font-bold uppercase tracking-[.18em] text-amber-700">Digital studio</div>
        <h1 className="max-w-xl text-5xl font-black leading-[.94] tracking-[-.055em] text-zinc-950 sm:text-6xl xl:text-7xl">
          We build digital<br/><span className="relative">experiences<svg className="absolute -bottom-3 left-0 h-3 w-full text-[#ffad0a]" viewBox="0 0 200 12" preserveAspectRatio="none"><path d="M2 8 Q100 1 198 7" fill="none" stroke="currentColor" strokeWidth="5" strokeLinecap="round"/></svg></span>
        </h1>
        <p className="mt-8 max-w-lg text-sm leading-7 text-zinc-600">Thiết kế và phát triển website hiện đại, rõ ràng và hiệu quả — từ ý tưởng đầu tiên đến sản phẩm vận hành thực tế.</p>
        ...
        <div className="mt-11 grid max-w-xl grid-cols-3 border border-zinc-900">
          {strengths.map((item, index) => (
            <div key={item.title} className={`group relative bg-white p-3 transition-colors hover:bg-[#fff2d4] sm:p-5 ${index < strengths.length - 1 ? "border-r border-zinc-900" : ""}`}>
              ...
              <div><h3 className="text-[8px] font-black uppercase leading-tight tracking-wide text-zinc-950 sm:text-[11px]">{item.title}</h3><p className="mt-1 text-[8px] leading-tight text-zinc-500 sm:text-[10px]">{item.detail}</p></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative mx-auto w-full max-w-[620px]">
        ...
        <div className="border border-zinc-900 bg-[#fff9ed] p-3 shadow-[10px_10px_0_#ffb21c]">
          <div className="mb-3 flex items-center justify-between border-b border-zinc-900 pb-3"><span className="text-[10px] font-black uppercase tracking-[.15em]">Creative developer</span><div className="flex gap-2"><i className="h-2 w-2 rounded-full bg-zinc-950"/><i className="h-2 w-2 rounded-full bg-[#ffad0a]"/></div></div>
