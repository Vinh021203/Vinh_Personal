...
export const Header = () => {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 h-[74px] border-b border-zinc-900 transition-all duration-300 ${scrolled ? "bg-white/95 shadow-[0_8px_30px_rgba(24,24,27,.08)] backdrop-blur-xl" : "bg-white"}`}>
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <nav className="hidden h-full items-center gap-1 lg:flex" aria-label="Điều hướng chính">
          {navigation.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`group relative flex h-full items-center px-3 text-[11px] font-extrabold uppercase tracking-[.12em] transition-colors ${active ? "text-zinc-950" : "text-zinc-500 hover:text-zinc-950"}`}>
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className={`hidden border border-zinc-900 px-5 py-2.5 text-xs font-black transition-all hover:-translate-y-0.5 sm:block ${contactActive ? "bg-zinc-950 text-white shadow-[3px_3px_0_#ffb21c]" : "bg-[#ffb21c] text-zinc-950 shadow-[3px_3px_0_#18181b]"}`}
          >
            {t.contact}
          </Link>
          ...
          {user ? (
            <div ref={accountRef} className="relative">
              ...
              <AnimatePresence>
                {accountOpen && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="absolute right-0 mt-2 w-48 border border-zinc-900 bg-white p-2 shadow-[5px_5px_0_#ffb21c]">
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="border border-zinc-900 bg-white p-2 transition-colors hover:bg-[#ffb21c] lg:hidden" aria-label="Mở menu">
            {mobileOpen ? <X /> : <Menu />}
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.nav initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute inset-x-0 top-[73px] grid border-b border-zinc-900 bg-[#fff9ed] p-5 shadow-xl lg:hidden">
            {navigation.map((item, index) => {
              const active = isActive(item.href);
              return (
                <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`flex items-center border-b border-zinc-300 px-3 py-3 text-sm font-black ${active ? "bg-[#ffb21c] text-zinc-950" : ""}`}>
                  <span className={`mr-4 ${active ? "text-zinc-950" : "text-[#d98200]"}`}>0{index + 1}</span>
                  {item.label}
                  {active && <span className="ml-auto h-2 w-2 rounded-full bg-zinc-950" />}
                </Link>
              );
            })}
            <Link href="/contact" aria-current={contactActive ? "page" : undefined} className={`mt-4 flex items-center justify-center border border-zinc-900 p-3 text-sm font-black ${contactActive ? "bg-zinc-950 text-white shadow-[3px_3px_0_#ffb21c]" : "bg-[#ffb21c] text-zinc-950 shadow-[3px_3px_0_#18181b]"}`}>
              {t.contact}
            </Link>
            {!user && (
              <div className="mt-4 flex gap-3">
                <Link href="/login" className="flex-1 border border-zinc-900 bg-white p-3 text-center text-sm font-bold">{t.login}</Link>
                <Link href="/register" className="flex-1 border border-zinc-900 bg-[#ffb21c] p-3 text-center text-sm font-bold">{t.register}</Link>
              </div>
            )}
          </motion.nav>
        )}
