...
// --- Copy toàn bộ TYPES, CONFIG, THEME vào đây ---
interface UserType {
  _id?: string;
  id?: string;
  name: string;
...
// --- Đổi tên thành export default function SettingsContent ---
export default function SettingsContent() {
    ...
    if (!loading && !currentUser) {
      ...
      if (!avatarFile) {
        ...
        if (currentUrl) {
          ...
          setDisplayAvatar(`${currentUrl}${separator}t=${Date.now()}`);
        } else {
          setDisplayAvatar("");
        }
      }
  ...
  const handleSaveAccount = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Lỗi khi cập nhật từ server");
      ...
      if (updatedUser.avatar) {
        ...
        setDisplayAvatar(`${updatedUser.avatar}${separator}t=${Date.now()}`);
      }
      await refreshUser();
