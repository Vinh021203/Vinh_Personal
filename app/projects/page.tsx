...
interface Project {
  ...
  category: string;
  technologies: string[];
  completedAt?: string; // Optional vì có thể chưa hoàn thành
  createdAt: string; // Fallback cho completedAt
  liveUrl?: string;
  featured?: boolean;
}
...
function ProjectCard({
  ...
  // Dùng completedAt hoặc createdAt để hiển thị năm
    ...
    >
      ...
      >
        <Image
          ...
          unoptimized // Thêm unoptimized để fix lỗi hiển thị ảnh Cloudinary nếu cần
        />
...
export default function ProjectsPage() {
  ...
  // Danh mục filter (Có thể lấy dynamic từ API nếu muốn, ở đây hardcode cho đẹp)
  const categories = [
    { id: "all", label: "Tất cả" },
    { id: "Website", label: "Website" }, // Cần match với dữ liệu trong DB
    ...
    const fetchProjects = async () => {
      try {
        const res = await fetch("/api/projects", {
          ...
          cache: "no-store", // Đảm bảo lấy data mới nhất
        });
  ...
  // Stats logic dựa trên dữ liệu thật
  const stats = [
    {
      icon: Rocket,
    ...
    },
    {
      icon: User,
      label: "Khách hàng",
      value: projects.length > 5 ? `${projects.length}+` : projects.length, // Mock logic
      color: "text-pink-500",
      bg: "bg-pink-100",
    },
    {
  ...
  return (
    <>
      <Head>
        <title>Portfolio Dự Án | VinhWorks</title>
        <meta
          name="description"
          content="Danh sách các dự án thực tế được thiết kế bởi VinhWorks."
        />
      </Head>
