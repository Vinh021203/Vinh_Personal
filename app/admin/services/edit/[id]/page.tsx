      ...
      <div className="grid min-h-[70vh] place-items-center">
        <div className="border border-zinc-950 bg-zinc-950 px-6 py-5 text-white shadow-[5px_5px_0_#ffb21c]">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em]">ang ti dch v</span>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="flex items-center gap-3 text-3xl font-black leading-none text-zinc-950 sm:text-4xl">Chnh sa dch v{isDirty && <span className="h-3 w-3 animate-pulse bg-[#ffb21c]" />}</h1>
      ...
      <form onSubmit={submit} className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-6">
          <FormPanel icon={Wrench} kicker="01  Nội dung" title="Thông tin dịch vụ">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <Field label="Slug"><input value={slug} onChange={(e) => { setSlug(e.target.value); touch(); }} className="admin-crud-input" /></Field>
              <Field label="Danh mục"><input value={category} onChange={(e) => { setCategory(e.target.value); touch(); }} className="admin-crud-input" /></Field>
            </div>
            <Field label="Mô tả ngắn"><textarea value={shortDescription} onChange={(e) => { setShortDescription(e.target.value); touch(); }} rows={3} maxLength={220} className="admin-crud-textarea" /></Field>
          ...
          <FormPanel icon={Sparkles} kicker="02  Nhận diện" title="Icon & ảnh minh hoạ">
            ...
            <Field label="Ảnh minh hoạ">{thumbnailPreview || currentImage ? <ImagePreview src={thumbnailPreview || currentImage} onRemove={() => { setThumbnail(null); setThumbnailPreview(""); setCurrentImage(""); touch(); }} /> : <UploadBox label="Tải ảnh dịch vụ" onChange={handleImageChange} />}</Field>
          </FormPanel>
          <FormPanel icon={Layers} kicker="03  Chi tiết" title="Quyền lợi & quy trình">
            <div className="grid gap-5 md:grid-cols-3">
              <Field label="Tính năng"><textarea value={features} onChange={(e) => { setFeatures(e.target.value); touch(); }} rows={5} className="admin-crud-textarea" /></Field>
              <Field label="Bàn giao"><textarea value={deliverables} onChange={(e) => { setDeliverables(e.target.value); touch(); }} rows={5} className="admin-crud-textarea" /></Field>
              <Field label="Quy trình"><textarea value={process} onChange={(e) => { setProcess(e.target.value); touch(); }} rows={5} className="admin-crud-textarea" /></Field>
            </div>
          </FormPanel>
          <FormPanel icon={Tag} kicker="04  SEO" title="Metadata & CTA">
            <div className="grid gap-5 md:grid-cols-2">
              <Field label="SEO title"><input value={seoTitle} onChange={(e) => { setSeoTitle(e.target.value); touch(); }} maxLength={70} className="admin-crud-input" /></Field>
              <Field label="OG image"><input value={ogImage} onChange={(e) => { setOgImage(e.target.value); touch(); }} className="admin-crud-input" /></Field>
              <Field label="CTA label"><input value={ctaLabel} onChange={(e) => { setCtaLabel(e.target.value); touch(); }} className="admin-crud-input" /></Field>
              <Field label="CTA link"><input value={ctaHref} onChange={(e) => { setCtaHref(e.target.value); touch(); }} className="admin-crud-input" /></Field>
            </div>
            <Field label="SEO description"><textarea value={seoDescription} onChange={(e) => { setSeoDescription(e.target.value); touch(); }} maxLength={170} rows={3} className="admin-crud-textarea" /></Field>
          </FormPanel>
        </div>
        ...
        <aside className="space-y-6 xl:sticky xl:top-28 xl:self-start">
          <FormPanel icon={Settings} kicker="05  Cấu hình" title="Thiết lập">
            ...
            <Field label="Giá hiển thị"><input value={priceLabel} onChange={(e) => { setPriceLabel(e.target.value); touch(); }} className="admin-crud-input" /></Field>
            <Field label="Giá khởi điểm (VNĐ)"><input type="number" value={startingPrice} onChange={(e) => { setStartingPrice(Number(e.target.value)); touch(); }} className="admin-crud-input" /></Field>
            <Field label="Giá nội bộ (VNĐ)"><input type="number" value={price} onChange={(e) => { setPrice(Number(e.target.value)); touch(); }} className="admin-crud-input" /></Field>
            <Field label="Thời gian thực hiện"><input value={timeline} onChange={(e) => { setTimeline(e.target.value); touch(); }} className="admin-crud-input" /></Field>
            <Field label="Thứ tự hiển thị"><input type="number" value={order} onChange={(e) => { setOrder(Number(e.target.value)); touch(); }} className="admin-crud-input" /></Field>
