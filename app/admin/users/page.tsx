...
interface User {
  ...
  lastLogin?: string;
  posts?: number; // Số liệu này Backend cần trả về (hoặc để 0 nếu chưa có logic đếm)
  projects?: number; // Tương tự
}
...
export default function UserListPage() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const formattedData = data.map((user: any) => ({
          ...
          // Nếu backend chưa có lastLogin, dùng tạm createdAt hoặc thời gian hiện tại
          lastLogin:
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      {/* 2. TOOLBAR (Đã xóa class 'sticky top-20') */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="w-24 h-24 p-1 transition-colors border-2 border-orange-200 border-dashed rounded-full group-hover:border-orange-400">
                    <div className="relative w-full h-full overflow-hidden rounded-full bg-slate-50">
                      <Image
                        ...
                        unoptimized // Fix lỗi hiển thị ảnh Cloudinary/External
                        onError={(e) => {
