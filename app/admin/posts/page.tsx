  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const enhancedData = data.map((post: any, index: number) => ({
          ...post,
          status: ["published", "draft", "archived"][index % 3] as
            | "published"
            | "draft"
            | "archived",
          views: Math.floor(Math.random() * 1000) + 50,
          featured: index < 2,
          tags: ["React", "Next.js", "TypeScript", "Web Dev"].slice(
            0,
        ...
        setPosts([
          {
            ...
            content: "Hướng dẫn chi tiết về Next.js 14...",
            status: "published",
            views: 1250,
            featured: true,
          ...
          },
          {
            ...
            date: "2024-01-10",
            thumbnail: "/placeholder.jpg",
            content: "Khám phá các kỹ thuật thiết kế...",
            status: "draft",
            views: 890,
            featured: false,
          ...
          >
            {currentData.map((post, index) => (
              ...
              >
                ...
                {/* Status Badge */}
                <div
                  className={`absolute top-4 right-4 z-10 px-2 py-1 text-xs font-medium rounded-full border flex items-center gap-1 ${getStatusColor(
                    post.status || "published"
                  )}`}
                >
                  {getStatusIcon(post.status || "published")}
                  {post.status === "published"
                    ? "Xuất bản"
                    : post.status === "draft"
                    ? "Nháp"
                ...
                {/* Content */}
                <div className="p-6">
                  <h3 className="mb-2 text-lg font-bold text-white transition-all line-clamp-2 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-purple-400 group-hover:to-blue-400 group-hover:bg-clip-text">
                    {post.title}
                  </h3>
                ...
                <tbody>
                  {currentData.map((post, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded-full border flex items-center gap-1 w-fit ${getStatusColor(
                            post.status || "published"
                          )}`}
                        >
                          {getStatusIcon(post.status || "published")}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-400">
                    ...
                    {previewPost.views && (
                      <div className="flex items-center gap-1">
                        <Eye size={14} />
                        <span>{previewPost.views} lượt xem</span>
                      </div>
                    )}
                  </div>
