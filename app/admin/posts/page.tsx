...
// --- TYPES ---
// Cập nhật Interface khớp với dữ liệu Backend trả về
interface Post {
  _id: string;
  title: string;
  author: string;
...
export default function PostListPage() {
  ...
  // --- API EFFECT (ĐÃ SỬA: LẤY DỮ LIỆU THẬT) ---
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await fetch("/api/posts", { cache: "no-store" }); // Đảm bảo luôn lấy dữ liệu mới nhất
        if (!res.ok) throw new Error("Failed to fetch posts");

        const data = await res.json();

        // Chuẩn hóa dữ liệu từ API để tránh lỗi undefined
        const formattedData = data.map((post: any) => ({
          ...
          // Ưu tiên dùng trường 'date' nếu có, nếu không dùng 'createdAt'
                ...
                >
                  ...
                  <div className="relative h-48 overflow-hidden bg-slate-50">
                    ...
                    <Image
                      ...
                      unoptimized // Fix lỗi hiển thị ảnh Cloudinary
                      onError={(e) => {
