...
// Generate realistic looking curve data for AreaChart
const generateChartData = (points: number, multiplier: number) => {
  ...
  for (let i = 0; i < points; i++) {
    // Random walk algorithm for smoother curves
...
const deviceData = [
  { name: "Mobile", value: 58, color: "#f97316" }, // Orange
  { name: "Desktop", value: 32, color: "#8b5cf6" }, // Violet
  { name: "Tablet", value: 10, color: "#94a3b8" }, // Slate
];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTimeout(() => setIsLoading(false), 1200); // Artistic delay
      }
    };
    fetchData();
  }, []);

  // 2. Generate Chart Data based on Period
  useEffect(() => {
