...
export default function AdminMessagesPage() {
  ...
  const filteredMessages = selectedUser
    ? messages.filter(
        m =>
          (m.senderId === selectedUser && (!m.receiverId || m.receiverId === adminId)) ||
  ...
  const handleReply = async () => {
    ...
    try {
      const res = await fetch('/api/messages', {
        ...
        body: JSON.stringify({ content: reply, receiverId: selectedUser, isAdmin: true }),
      });
  ...
  return (
    <section className="relative flex h-[calc(100vh-64px)] bg-gradient-to-br from-gray-900 to-black text-white">
      ...
      <div className="absolute z-20 top-2 left-4 md:hidden">
        ...
        >
          <User size={16} /> {selectedUser ? 'Đổi người dùng' : 'Chọn người dùng'}
        </button>
      </div>
      ...
      <div className="flex flex-col flex-1 p-4 overflow-hidden pt-14 md:pt-4 sm:p-6">
        <h2 className="mb-6 text-lg font-bold text-teal-400">
          {selectedUser
            ? `Tin nhắn với: ${uniqueUsers.find((u) => u[0] === selectedUser)?.[1]}`
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 text-xs text-white/50">
                    {msg.senderName} • {new Date(msg.createdAt).toLocaleString('vi-VN')}
                  </div>
                </div>
              );
            })
